            ...
            <section class="col-10">
                <section class="card p-4 shadow rounded bg-light">
                    ...
                    <section class="text-center">
                        <img src="{{ asset('imagenesTienda/iconos/logoAdopcion.jpg') }}" alt="Adopción Responsable"
        ...
        <section class="row justify-content-center">
            <section class="col-10">
                <section class="card p-4 shadow rounded">
                    ...
                    <form action="{{ url('pawsTails/animales') }}" method="POST" class="d-flex p-3">
                        @csrf
                        <section class="form-group d-flex">
                            ...
                            <select id="refugio" name="refugio" class="form-control color-enfoque" required>
                                ...
                                @foreach ($datosRefugios as $refugio)
                                    <option value="{{ $refugio->id }}"
                                        @if (session('refugioBuscado') == $refugio->id) selected @endif>
                                        {{ $refugio->nombre }}</option>
                                @endforeach
                            </select>
                        </section>
                    ...
                    <section class="row">
                        ...
                        @foreach ($animales as $animal)
                            @if (!$refugioBuscado || $refugioBuscado == $animal->usuario->id)
                                <section class="col-md-4 mb-4">
                                    <section class="card h-100">
                                        <section class="card-info">
                                            <section class="card-contenido">
                                                <h5 class="card-title">{{ $animal->nombre }}</h5>
                                                <p class="card-text"><b>{{ strtoupper($animal->especie) }}</b></p>
                                                <p class="card-text"><b>Refugio: </b>{{ $animal->usuario->nombre }}
                                                </p>
                                                <p class="card-text"><b>Contacto: </b>{{ $animal->usuario->correo }}
                                                </p>
                                                <p class="card-text">{{ $animal->descripcion }}</p>
                                                <a href="/pawsTails/adoptar/{{ $animal->id }}"
                                                    class="btn btn-formulario">Adoptar<img
                                                        src="{{ asset('imagenesTienda/iconos/heart-handshake.png') }}"
                                                ...
                                                @if (session('rol_usuario') == 'administrador' ||
                                                    ...
                                                    <a href="/pawsTails/editarAnimal/{{ $animal->id }}"
                                                        class="btn btn-primary">Editar<img
                                                            src="{{ asset('imagenesTienda/iconos/edit.png') }}"
                                                            width="35" height="35" class="img-fluid p-1" alt="Icono de un lápiz"></a>
                                                    <a href="/pawsTails/borrarAnimal/{{ $animal->id }}"
                                                        class="btn btn-danger m-2"><img
                                                            src="{{ asset('imagenesTienda/iconos/trash.png') }}"
                                                            width="35" height="35" class="img-fluid p-1"
                                                            title="Eliminar animal" alt="Icono de una papelera"></a>
                                                @endif
                                            </section>
                                            <section class="card-imagen"
                                                style="background-image: url('{{ Storage::url('public/imagenesAnimales/' . $animal->imagen) }}');">
                                            </section>
                                        </section>
                                    </section>
                                </section>
